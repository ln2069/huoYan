...
async function handleExport() {
  try {
    ...
    const blob = await repositories.export.exportCsv({
      type: "evidence",
      case_id: selectedCaseId.value ? Number(selectedCaseId.value) : undefined
    });
  ...
  <div class="space-y-6">
    ...
    <div class="app-card p-8 shadow-md rounded-xl">
      ...
      <div class="flex gap-4 mb-6 items-center">
        ...
        <el-input v-model="searchText" placeholder="搜索证据内容" :prefix-icon="Search" class="!w-[280px]" size="default"
          clearable />
      ...
      <el-table :data="filteredList.slice((currentPage - 1) * pageSize, currentPage * pageSize)" stripe
        ...
        <el-table-column label="相关人员" min-width="160">
          <template #default="{ row }">
            <div v-if="row.initiator || row.receiver" class="flex flex-col gap-1">
              <span v-if="row.initiator" class="text-sm">发起: <span class="font-bold text-[#1A3A5C]">{{ isMasked ?
                maskName(row.initiator) : row.initiator }}</span></span>
              <span v-if="row.receiver" class="text-sm">接收: <span class="font-bold text-[#1A3A5C]">{{ isMasked ?
                maskName(row.receiver) : row.receiver }}</span></span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="关键词" min-width="150">
          <template #default="{ row }">
            <div class="flex flex-wrap gap-1.5">
              <span v-for="k in row.hit_keywords" :key="k"
        ...
        <el-table-column prop="severity_level" label="严重程度" min-width="110" align="center">
          <template #default="{ row }">
            <span v-if="row.severity_level" class="px-2 py-0.5 rounded-full text-[10px] font-bold border" :style="{
